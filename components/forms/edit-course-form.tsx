...
import { AddCourseFormValues, addCourseSchema } from "./add-course-form";


// Helper component for syllabus lessons
...
export default function EditCourseForm({ course }: EditCourseFormProps) {
    ...
    const transformCourseToFormValues = (course: ICourse): AddCourseFormValues => {
        return {
            title: course.title || "",
            ...
            whatYouWillLearn: course.whatYouWillLearn?.map(item => ({ value: item })) || [{ value: "" }],
            level: course.level || "beginner",
            duration: course.duration || "",
            language: course.language || "English",
            category: course.category || "",
            status: course.status || "published",
            requirements: course.requirements?.map(req => ({ value: req })) || [{ value: "" }],
            syllabus: course.syllabus?.map(module => ({
                ...
                })) || [{ title: "", content: "", duration: 0, videoUrl: "" }]
            })) || [{
                title: "",
                description: "",
    ...
    const form = useForm<AddCourseFormValues>({
        ...
        defaultValues: transformCourseToFormValues(course),
        mode: "onBlur",
    });
    ...
    useEffect(() => {
        reset(transformCourseToFormValues(course));
    }, [course, reset]);
    ...
    // Add this after the field arrays declarations
    useEffect(() => {
    ...
    const onSubmit = async (values: AddCourseFormValues) => {
        ...
        try {
            const response = await axios.put(`/api/admin/courses/${course._id}`, cleanedValues);
        ...
        <Form {...form}>
            <form onSubmit={handleSubmit(onSubmit)} className="max-w-5xl mx-auto space-y-10 p-6 bg-white rounded-2xl shadow-sm">
                <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">Update your course details below.</p>
                </div>
                ...
                {/* Social proof: fake students, rating and sample reviews (optional) */}
                <section className="space-y-4">
                    <div className="flex items-center justify-between">
